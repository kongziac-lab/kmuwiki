...
function ResultView({ result }: { result: CombinedResult }) {
  ...
  const [savedWorkflow, setSavedWorkflow] = useState<SaveWorkflowResponse | null>(null);
  ...
  async function saveWorkflow() {
    if (workflowNodes.length === 0 || saveBusy) return;
    setSaveBusy(true);
    setSaveErr("");
    try {
      const token = await getAccessToken();
      if (!token) throw new Error("로그인 세션이 없습니다.");
      const title = `${result.query} 업무흐름도`;
      const saved = await postJson<SaveWorkflowResponse>("/api/workflows", token, {
        title,
        query: result.query,
        ...
        source: {
          calendar_draft_count: calendarDrafts.length,
          classification_count: classifications.length,
        },
      });
      setSavedWorkflow(saved);
    } catch (error: unknown) {
      setSaveErr(error instanceof Error ? error.message : "업무흐름도 저장 실패");
    } finally {
      setSaveBusy(false);
    }
  }
  ...
  return (
    <div style={results}>
      ...
      <section className="glass" style={widePanel}>
        <div style={workflowSaveRow}>
          <PanelHeader title="업무흐름도" meta={`${workflowNodes.length || timeline.length}단계`} />
          <button className="btn btn-ghost" style={compactButton} onClick={saveWorkflow} disabled={workflowNodes.length === 0 || saveBusy}>
            {saveBusy ? "저장 중..." : "업무흐름도 저장"}
          </button>
        </div>
        ...
        {savedWorkflow && (
          <p style={itemMeta}>
            저장됨: <a href={savedWorkflow.url}>{savedWorkflow.title}</a>
          </p>
        )}
...
const workflowSaveRow: CSSProperties = {
  display: "flex",

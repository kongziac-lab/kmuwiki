...
- [x] **샘플 ZIP 구조 분석 완료** (2026-06): ZIP=결재 1건, 최상위 폴더(제목) 안에 본문 기안문 PDF(폴더명과 동일) + 붙임(hwp/xls/pdf/html/mht). 파일명은 CP949.
  ...
  - 본문 기안문 PDF는 **텍스트 기반**(pdfplumber). **문서번호는 항상 시행란에 존재**(기관 확인).
    단 부서명은 공백 포함(예: "장춘대학 계명학원 행정팀")이고 긴 부서명은 셀에서 줄바꿈되어 번호가
    다음 줄로 내려가므로, **단어 좌표로 시행란을 복원**해 추출(`_docno_from_words`). 6개 실문서 전부 추출 확인.
  - 기안문 선별: 폴더명 일치 PDF 우선, 없으면 제목 토큰 겹침 최대 + 시행번호 보유 PDF(ISEP처럼 관련 문서가 섞인 ZIP 대응).
